...
class PwnCert:
    ...
    def find(self, output: str = None, vulnerable: bool = False, 
        ...
        if output:
            ...
            # Se não especificar output, usar um nome padrão
            cmd.extend(['-output', 'certipy_find_results'])
...
Exemplos:
  ...
  %(prog)s find -u 'admin user' -p 'Pass@123' -d domain.local --vulnerable
...
Notas:
  ...
  - Exemplos: -u 'admin user' ou -p 'Pass@123!' ou -d 'corp.local'
        """
    )
    ...
    if not args.password and not args.hashes:
        print("\n[!] Erro: Forneça -p/--password ou -H/--hashes")
        print("[!] Exemplo: -p 'Password123' ou -H 'LM:NT'")
        return 1
    ...
    if args.debug:
        ...
        print(f"  Hashes: {'***' if args.hashes else 'None'}")
        print(f"  DC-IP: {args.dc_ip if args.dc_ip else 'None'}")
